...
def main(args: argparse.Namespace) -> tuple[np.ndarray, np.ndarray]:
    ...
    for i, col in enumerate(train_data.T):
        if all(x.is_integer() for x in col):
    ...
    transformer = ColumnTransformer([
            ("categ", enc1, categorical_columns),
            ("nr", enc2, numerical_columns)
        ]
    )

    # train_processed = transformer.fit_transform(train_data)
    # test_processed = transformer.transform(test_data)
    ...
    train_final = pipeline.fit_transform(train_data)
    test_final=pipeline.transform(test_data)
